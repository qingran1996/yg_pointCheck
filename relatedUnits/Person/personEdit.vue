  ...
  methods: {
      ...
      this.$axios.post('/p/personJobInfo/getByPersonIdAndUnitId', checkPar).then(res => {
        if (res.data.code === 0) {
          ...
          if (result.workType === ''){
            ...
            this.infoData.workType = result.workType.split(',')
          }
    ...
    async axiosUpdate() {
      let parOne = {
        ...
        code: this.tPerson.code,
        phone: this.infoData.phone,
        email: this.infoData.email,
        idcard: this.infoData.idcard,
        state: this.tPerson.state,
        userTypeCode: this.tPerson.userTypeCode,
        orgCode: this.tPerson.orgCode
      }
      let src=''
      ...
      let backThing = ''
      await this.$axios.post('/p/person/update', parOne).then(res => {
        if (res.data.code !== 0) {
