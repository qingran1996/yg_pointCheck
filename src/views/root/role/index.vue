    ...
    <vxe-modal v-model="showTabs" title="新增&保存" width="60%" min-width="600"
      ...
      <template #default>
        <vxe-form :data="group" :rules="groupRules"  title-colon title-align="right" title-width="150"
                  ref="tForm" @submit="groupSumbit">
          <vxe-form-item title="标签名称" field="name" span="12" :item-render="{}">
            <template #default="scope">
              <vxe-input v-model="group.name" @input="$refs.tForm.updateStatus(scope)"
                         placeholder="请输入名称" clearable></vxe-input>
            </template>
          </vxe-form-item>
          <vxe-form-item title="标签编码" field="groupCode" span="12" :item-render="{}">
            <template #default="scope">
              <vxe-input v-model="group.groupCode" @input="$refs.tForm.updateStatus(scope)"
                         placeholder="请输入编码" clearable></vxe-input>
            </template>
          </vxe-form-item>
        ...
        <el-table :data="tabList" @selection-change="selectionchange"
          ...
          <el-table-column prop="name" show-overflow-tooltip label="名称" min-width="180" align="center">
            <template v-slot="spoce">
              {{spoce.row.name}}
              <i class="el-icon-edit" style="color:#1890ff;" @click="open(spoce.row)"></i>
            </template>
    ...
    methods: {
      ...
      getDataList() {
        this.roleList(this.jsonData).then(res => {
          ...
          for (var i = 0; i < data.length; i++) {
            data[i]['cid'] = this.jsonData.current * this.jsonData.size - this
              .jsonData.size + i + 1
          }
          this.tableData = data
      ...
      pullMenu(code){
        this.getAllMenuByRole({roleCode:code}).then(res => {
          console.log('pullMenu拉取菜单', res.data)
          if (res.data.code == 0){
      ...
      pullRole(code){
        this.getScope(code).then(res => {
          console.log('pullRole拉取数据', res.data.data)
          // console.log('ID',res.data.data[0].id )
          this.demoRoleID=false
          if (res.data.data.length!=0&&res.data.data[0].id!=''){
            this.demoRoleID = res.data.data[0].id
            console.log('ID',this.demoRoleID )
          }
          if (res.data.code == 0){
            ...
            if (res.data.data.length!=0){
              ...
              let Menu2 = []
                ...
                let arr =JSON.parse(Data.scopeData).orgCode
                let arr2 =JSON.parse(Data.scopeData).major
