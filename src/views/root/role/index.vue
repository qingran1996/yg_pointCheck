<template>
	<div class="domain">
		<el-card class="box-card" shadow="always">
			<el-form :inline="true" :model="jsonData" class="demo-form-inline">
				...
				<el-form-item>
					...
					<el-button icon="el-icon-circle-plus-outline" size="small" type="primary" @click="addRoute">新建角色
					</el-button>
				</el-form-item>
			</el-form>
		</el-card>
			...
			<el-row style="margin-bottom: 5px;">
				<p>
					<vxe-radio-group v-model="selectTab">
						<vxe-radio-button :label="o.groupCode" v-for="o in tabList" :key="o.groupCode"><i
								class="el-icon-user"></i>{{o.name}}
						</vxe-radio-button>
					</vxe-radio-group>
		...
		<vxe-modal v-model="showEdit" :title="selectRow ? '编辑&保存' : '新增&保存'" width="60%" height="50%" min-width="600"
		...
		<vxe-modal v-model="showTabs" title="新增&保存" width="60%" min-width="600" resize destroy-on-close>
			<template #default>
				<vxe-form :data="group" :rules="groupRules" title-colon title-align="right" title-width="150"
					...
					<vxe-form-item title="标签名称" field="name" span="12" :item-render="{}">
						<template #default="scope">
							<vxe-input v-model="group.name" @input="$refs.xForm.updateStatus(scope)" placeholder="请输入名称"
								clearable></vxe-input>
						</template>
					</vxe-form-item>
		...
		<vxe-modal v-model="showTabs2" title="标签编辑" width="60%" min-width="600" resize destroy-on-close>
			<template #default>
				<el-table :data="tabList" @selection-change="selectionchange" :row-key="getRowKeys" border
					style="width: 100%" :max-height="tableHeight">
					<el-table-column type="index" width="100" label="序号" align="center" fixed>
					</el-table-column>
					<el-table-column prop="groupCode" show-overflow-tooltip label="角色编码" min-width="180"
						align="center" />
		...
		<vxe-modal v-model="showMenu" title="权限设置" width="60%" min-width="600" resize destroy-on-close
			@close="closemodel">
			<template #default>
				<div class="info">
					<p>角色名称：设备处-设备处处长</p>
				...
				<el-card class="box-card _box" v-show="tabIndex=='1'">
					<div class="_bar">
						...
						<el-tree :data="WEBTreeData" :props="defaultProps2" show-checkbox node-key="code" ref="roles"
							:default-checked-keys="celkALL">
						</el-tree>
					</div>
					<div class="_bar">
						<div class="title">APP权限</div>
						<el-tree :data="APPTreeData" :props="defaultProps2" show-checkbox node-key="code">
						</el-tree>
					</div>
				...
				<el-card class="box-card _box" v-show="tabIndex=='2'">
					<div class="_bar">
						...
						<el-tree ref="roles2" :data="pointData" :props="defaultProps3" show-checkbox node-key="orgCode"
							:default-checked-keys="celkSum" @check-change="displineC">
						</el-tree>
					</div>
					<div class="_bar">
						<div class="title">专业</div>
						<el-tree ref="roles4" :data="mdJson" :props="defaultProps2" show-checkbox node-key="id"
							@check-change="displineChange" :default-checked-keys="celkSum2">
						</el-tree>
					</div>
			...
			getDataList() {
				this.roleList(this.jsonData).then(res => {
					for (var i = 0; i < res.data.data.records.length; i++) {
						res.data.data.records[i]['cid'] = this.jsonData.current * this.jsonData.size - this.jsonData.size + i + 1
					}
