<template>
	<div class="role_look_tree">
		<el-tree v-if="isshow" v-loading="loading" :data="treeData" ref="tree" show-checkbox node-key="id" :default-expanded-keys="expandshow"
						...
						setTimeout(function() {
							if (data.length != 0) { //如果请求数据不为空
								...
								for (var i = 0; i < data.length; i++) {
									if (data[i].pid == 0 && data[i].appId==1) {
										that.treeData.push(data[i])
									}
									// if (data[i].pid == 0 && data[i].appId==3) {
									// 	data[i].disabled = true
									// 	that.treeData.push(data[i])
									// 	that.choosedata.push(data[i].id)
									// 	// console.log(data[i])
									// }
								}
								for (let i = 0; i < that.treeData.length; i++) {
									...
									for (let j = 0; j < data.length; j++) {
										if (that.treeData[i].id == data[j].pid && data[i].appId==1) {
