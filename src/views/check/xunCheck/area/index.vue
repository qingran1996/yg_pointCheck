	...
	<div class="xunCheckArea">
		...
		<el-card shadow="always">
			...
			<el-table v-loading="loading" ref="multipleTable" :data="tableData" border style="width: 100%"
				...
				<el-table-column prop="planStart" label="任务开始时间" min-width="180" align="center" show-overflow-tooltip />
				<el-table-column prop="planEnd" label="任务结束时间" min-width="180" align="center" show-overflow-tooltip />
				...
				<el-table-column prop="result" label="点检状态" min-width="180" align="center" show-overflow-tooltip>
					<template v-slot="scope">
						<div class="action">
							<span v-if="scope.row.result===0">未开始</span>
							<span v-else-if="scope.row.result===1">已完成</span>
							<span v-else-if="scope.row.result===2">漏检</span>
							<span v-else-if="scope.row.result===3">已取消</span>
							<span v-else>正在点巡检</span>
						</div>
					</template>
				</el-table-column>
				<el-table-column prop="photo" label="扫码图" fixed="right" min-width="180" align="center" show-overflow-tooltip />
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取table数据
			getTable() {
				this.pointCheckAreaTaskData(this.jsonData).then(res => {
					if (res.data.code === 0) { //查询到数据
						//console.log(res)
