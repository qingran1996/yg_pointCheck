...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        name: [{ required: true, message: '请输入相关单位名称', trigger: ['change', 'blur'] },
    ...
    updateInfo() {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          this.$axios.post('/p/unit/update',editPar).then(res=>{
            if (res.data.code === 0){
