...
<!--        <el-button type="danger" :disabled="isDelete" @click="deleteItems">删除</el-button>-->
      ...
      <el-table v-loading="loading" :data="tableData" border style="width: 100%" :max-height="tableHeight"
                 row-key="index" ref="multipleTable">
...
<script>
import ownCompanyEdit from '@/views/person/organization/ownCompanyEdit'
import ownCompanyDetail from '@/views/person/organization/ownCompanyDetail'
export default {
  components: {ownCompanyEdit,ownCompanyDetail},
  data() {
    return {
      loading: false,
      //isDelete: true,
  ...
  methods: {
    lookList(row) {
      this.$bus.emit('ownCompanyLookDetail',row)
    },
    //编辑
    edit(row) {
      let item = row
      this.$bus.emit('ownCompanyEdit', item)
    },
    //获取我方单位信息
    getTableData() {
      this.loading = true
      this.$axios.post('/p/unit/query', this.unitJson).then(res => {
        if (res.data.code === 0) {
          //console.log(res)
  ...
  destroyed() {
    this.$bus.off('ownCompanyEdit')
    this.$bus.off('renderTable')
    this.$bus.off('ownCompanyLookDetail')
  }
}
</script>
<style scoped>
