...
//线上头http://58.210.247.252:10005
const baseURL = process.env.NODE_ENV != 'development' ? 'http://apiygtpm.yong-gang.cn:8260/' :
	'http://192.168.10.152:10005' //开发模式 为152,build时为153
...
//   // "Operation-User": localStorage.username
// }
...
axios.interceptors.request.use(
	config => {
		...
		if (localStorage.eleToken) {
			...
			// config.headers.set('OperationUser',localStorage.userName)
			// config.headers.OperationUser = localStorage.userName
			config.headers['OperationUser'] = localStorage.username
		}
